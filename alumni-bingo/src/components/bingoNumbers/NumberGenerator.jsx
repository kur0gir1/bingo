...
export default function NumberGenerator({ config }) {
        ...
        <div className="grid grid-cols-5 gap-1 bg-black p-2 rounded-xl w-full h-full">
          {grid.map((row, r) => row.map((on, c) => (
            <div
              ...
              style={on ? { background: color, boxShadow: 'inset 0 0 0 1px rgba(0,0,0,0.12)' } : { background: 'transparent', border: '1px solid rgba(255,255,255,0.06)' }}
            />
          )))}
        </div>
      </div>
  ...
  const makeGrid = (fill = false) => Array.from({ length: 5 }, () => Array(5).fill(fill));
  const defaultGridFor = (pattern) => {
    ...
    switch ((pattern || '').toLowerCase()) {
      case 'diamond':
        ...
        for (let r = 0; r < 5; r++) for (let c = 0; c < 5; c++) if (Math.abs(r - 2) + Math.abs(c - 2) === 2) g[r][c] = true;
        return g;
      case 'cross':
        // 'cross' as an X: both diagonals
        for (let i = 0; i < 5; i++) { g[i][i] = true; g[i][4 - i] = true; }
        return g;
      case 'outside':
        for (let r = 0; r < 5; r++) for (let c = 0; c < 5; c++) if (r === 0 || r === 4 || c === 0 || c === 4) g[r][c] = true;
        return g;
      case 'diagonal':
        // bottom-right triangle (opposite side): cells where r + c >= 4
        for (let r = 0; r < 5; r++) for (let c = 0; c < 5; c++) if (r + c >= 4) g[r][c] = true;
        return g;
      case 'blackout':
  ...
  <div className="flex flex-col items-center w-full max-w-5xl mx-auto h-full justify-center px-2">
      <div className="text-l text-white py-2">LA CONSOLACION COLLEGE BACOLOD ALUMNI ASSOCIATION</div>
      <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white">BINGO SOCIAL</div>
      <div className="text-white text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-6 mt-1 sm:mt-2 tracking-wide text-center">PREVIOUS DRAWS:</div>
      <div className="flex items-start justify-center gap-4 sm:gap-6 md:gap-8 mb-4 sm:mb-8 min-h-[4.5rem] sm:min-h-[6rem] w-full">
        {previousDraws.map((n, i) => (
          ...
          >
            <span className="text-2xl sm:text-4xl md:text-5xl font-bold">{getLabel(n)} {n}</span>
          </div>
        ))}
      </div>
  ...
  <div className="flex flex-col md:flex-row items-center justify-center mb-6 sm:mb-12 w-full gap-4">
        ...
        <div className="flex-shrink-0 flex items-center justify-center md:justify-start" style={{ width: '100%', maxWidth: '720px' }}>
          <div style={{ width: '36vw', maxWidth: '380px', minWidth: '140px', marginLeft: '-6rem' }}>
        ...
        <div className="flex-1 flex items-center justify-center" style={{ minWidth: 0 }}>
          {last ? (
            <div className="flex flex-row items-end gap-4 sm:gap-8 w-full justify-center" style={{ transform: 'translateX(-2rem)' }}>
              <span className="tabular-nums text-white text-[6.25rem] sm:text-[11.25rem] md:text-[15.25rem] font-extrabold leading-none drop-shadow-2xl">{getLabel(last)}</span>
              <span className="tabular-nums text-white text-[6.25rem] sm:text-[11.25rem] md:text-[15.25rem] font-extrabold leading-none drop-shadow-2xl">{last}</span>
            </div>
          ) : (
            <div className="text-white text-3xl sm:text-5xl font-bold text-center">Press START GAME to start</div>
          )}
        </div>
      </div>
  ...
  <WinnerNumbers open={showWinnerModal} onClose={() => setShowWinnerModal(false)} numbers={drawn} resetSelected={resetSelected} color={extractPrimaryColor(config.gradient)} />
    </div>
  );
}
