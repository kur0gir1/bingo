...
export default function Loader({ show = false, from = 1, to = 75, duration = 1500, onFinish, gradient }) {
  ...
  style={{ backgroundColor: gradient ? `${gradient}` : 'rgba(0,0,0,0.5)' }}
    >
      <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-6 w-[min(420px,90vw)] text-center shadow-2xl"
        ...
        <div className="text-black select-none">
          ...
          <div className="mt-4 text-sm text-black">Switching games…</div>

          <div className="mt-6 h-2 bg-white/10 rounded-full overflow-hidden shadow-sm"
